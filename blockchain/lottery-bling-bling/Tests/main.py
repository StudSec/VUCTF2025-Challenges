# python3 main.py --flag CTF{l0773ry_l0v3r} --connection-string localhost:1337 --handout-path ../Handout/src --deployment-path ../Source/src
# paar keer opnieuw proberen (ligt aan bruteforce)
...
def run_test(flag, connection_string=None, handout_path=None, deployment_path=None):
    ...
    if result["DEPLOYMENT_WORKING"]:
        ...
        bashCommand = f"/home/shinxy/.foundry/bin/forge script script/Solve.s.sol --rpc-url {rpc_endpoint.decode().strip()} --tc Solve --broadcast -vvvvv"
    ...
    if not {"LotteryBlingBling.sol", "Setup.sol"}.issubset(set(os.listdir(handout_path))):
        result["HANDOUT_CORRECT"] = "Missing LotteryBlingBling.sol Setup.sol"
    else:
        _, missmatch, _ = filecmp.cmpfiles(handout_path, deployment_path,
                                           ["LotteryBlingBling.sol", "Setup.sol"], shallow=False)
