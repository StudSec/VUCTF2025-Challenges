...
HOSTNAME="127.0.0.1"
PORT=1569
FLAG="VUCTF{b0f_F0rM4T_p1v0T_4ll_4T_0NCe}"
TEAM_UUID=""
...
# if [ -n "$TEAM_UUID" ]; then
#   docker compose -p "pwn_toaster_$TEAM_UUID" up --build -d
# else
#   docker compose up --build -d
# fi
docker build -t pwn_core_master .
docker run -dp $PORT:1569 --privileged --name core_master pwn_core_master
