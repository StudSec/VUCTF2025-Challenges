...
FLAG = os.getenv("FLAG", "STUDSEC{place-holder}")
...
def sendTransaction(web3: Web3, tx: Dict) -> Optional[TxReceipt]:
    if "gas" not in tx:
        tx["gas"] = 30_000_000  # Increase gas limit
    ...
    if rcpt.status != 1:
        # Get the revert reason using eth.call
        try:
            revert_reason = web3.eth.call(tx)  # Simulates tx to get error
            print(f"Transaction failed with reason: {web3.to_text(revert_reason)}")
        except Exception as e:
            print(f"Transaction failed, but could not retrieve revert reason: {e}")

        raise Exception(f"Transaction failed: {rcpt}")
...
def new_launch_instance_action(
    ...
    def action() -> int:
        ...
        print(f"uuid:           {uuid}")
        print(f"rpc endpoint:   http://{PUBLIC_IP}/{uuid}")
