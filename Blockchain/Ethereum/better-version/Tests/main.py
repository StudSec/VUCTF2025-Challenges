# python3 main.py --flag CTF{thats_really_a_better_version} --connection-string localhost:1337 --handout-path ../Handout/src --deployment-path ../Source/src
...
def run_test(flag, connection_string=None, handout_path=None, deployment_path=None):
    ...
    if result["DEPLOYMENT_WORKING"]:
        ...
        bashCommand = f"/Users/wout/.foundry/bin/forge script script/Solve.s.sol --rpc-url {rpc_endpoint.decode().strip()} --tc Solve --broadcast"
    ...
    if not {"BetterVersion.sol", "UUPSProxy.sol", "Setup.sol"}.issubset(set(os.listdir(handout_path))):
        result["HANDOUT_CORRECT"] = "Missing BetterVersion.sol, UUPSProxy.sol or Setup.sol"
    else:
        _, missmatch, _ = filecmp.cmpfiles(handout_path, deployment_path,
                                           ["BetterVersion.sol", "UUPSProxy.sol", "Setup.sol"], shallow=False)
