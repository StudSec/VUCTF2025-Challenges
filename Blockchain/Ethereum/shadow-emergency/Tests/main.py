# python3 main.py --flag CTF{r33n7r4ncy_3m3r63ncy} --connection-string localhost:1337 --handout-path ../Handout/src --deployment-path ../Source/src
...
def run_test(flag, connection_string=None, handout_path=None, deployment_path=None):
    ...
    if not {"ShadowEmergency.sol", "Setup.sol"}.issubset(set(os.listdir(handout_path))):
        result["HANDOUT_CORRECT"] = "Missing ShadowEmergency.sol or Setup.sol"
    else:
        _, missmatch, _ = filecmp.cmpfiles(handout_path, deployment_path,
                                           ["ShadowEmergency.sol", "Setup.sol"], shallow=False)
